...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if hasattr(geojson_cache, 'client') and hasattr(geojson_cache.client, 'get_client'):
            ...
            if not isinstance(redis_client, redis.Redis):
                 raise CommandError("Retrieved client is not a recognized 'redis-py' client.")

        except Exception as e:
            raise CommandError(f"Error accessing Redis client for cache '{cache_alias}': {e}")

        self.stdout.write(f"Monitoring GeoJSON cache usage in Redis DB {redis_client.connection_pool.connection_kwargs.get('db', '?')}...")
        ...
        try:
            ...
            db_keys = info.get(f"db{redis_client.connection_pool.connection_kwargs.get('db', 0)}", {}).get('keys', 'N/A')
            ...
            for key in redis_client.scan_iter(match='*geojson*', count=1000):
                ...
                try:
                    ...
                    if size is not None:
                     ...
                     self.stdout.write(self.style.WARNING(f"Could not get memory usage for key '{key_str}': {mem_err}"))
            ...
            if key_sizes:
                ...
                for key, size in key_sizes[:10]:
                    self.stdout.write(f"  {key}: {size / 1024:.2f} KB") # Convert bytes to KB

            self.stdout.write('-' * 50)
            self.stdout.write(self.style.SUCCESS('Cache monitoring check complete.'))

        except redis.RedisError as e:
            raise CommandError(f"Redis error during monitoring: {e}")
        except Exception as e:
