...
class GeoJSONCachingTests(TestCase):
    ...
    def test_cache_invalidation_on_save(self):
        ...
        self.assertIsNotNone(self.geojson_cache.get(detail_key), "Detail key not cached")
        ...
        self.assertIsNone(self.geojson_cache.get(detail_key), "Specific detail key not invalidated")
        # The pattern delete ('region_geojson:*') should have cleared the list key too
        self.assertIsNone(self.geojson_cache.get(list_key), "List key not invalidated by pattern")
    ...
    def test_cache_invalidation_on_delete(self):
        ...
        self.assertIsNone(self.geojson_cache.get(detail_key), "Detail key not invalidated on delete")
        self.assertIsNone(self.geojson_cache.get(list_key), "List key not invalidated by pattern on delete")
    ...
    def test_monitor_command(self):
        ...
        self.assertIn("GeoJSON keys found (via SCAN):", output)  # Check for SCAN usage indicator
    ...
    def test_clear_cache_command(self):
        ...
        # 2. Run clear command
        out = io.StringIO()
        call_command('clear_geojson_cache', pattern='*region_geojson*', stdout=out)  # Clear region keys
