...
class CatchmentViewSet(CachedGeoJSONMixin, AutoPermModelViewSet):
    ...
    def _version_aggregates(self):
        """Catchment geometries derive from the related Region geometry, so the
        version additionally tracks the related Region modification time."""
        return {
...
class NutsRegionViewSet(CachedGeoJSONMixin, AutoPermModelViewSet):
    queryset = (
        ...
        .prefetch_related(
            "regionattributevalue_set__property",
