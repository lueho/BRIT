...
def get_dynamic_model(dataset):
    ...
    display_fields = [
        f.strip() for f in (dataset.display_fields or "").split(",") if f.strip()
    ]
    filter_fields = [
        f.strip() for f in (dataset.filter_fields or "").split(",") if f.strip()
    ]
    ...
    with connection.cursor() as cursor:
        try:
            ...
            raise ImproperlyConfigured(
                f"Table '{table_name}' not found: {err}"
            ) from err
    ...
    fields = {
        "__module__": __name__,
        "Meta": type(
            "Meta", (), {"managed": False, "db_table": table_name, "app_label": "maps"}
        ),
    }
    for col in desc:
