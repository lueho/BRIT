...
class Process(NamedUserCreatedObject):
    ...
    def _material_links_for_role(self, role: "ProcessMaterial.Role"):
        ...
        if cache and "process_materials" in cache:
            links = [
                link
                for link in cache["process_materials"]
                if link.role == role
            ]
        if links is None:
            links = list(
    ...
    def operating_parameters_for(self, parameter: "ProcessOperatingParameter.Parameter"):
        ...
        return self.operating_parameters.filter(parameter=parameter).order_by("order", "id")
...
class ProcessMaterial(models.Model):
    ...
    def clean(self):
        ...
        if self.quantity_unit and self.quantity_value is None:
            raise ValidationError(
                {"quantity_value": _("Provide a quantity value when a unit is selected.")}
            )
    ...
    def clean(self):
        ...
        if self.resource_type == self.ResourceType.DOCUMENT:
            ...
            if not cleaned_url:
                ...
                if self.resource_type == self.ResourceType.EXTERNAL and not parsed.scheme:
                    errors["url"] = _(
