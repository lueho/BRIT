...
class ScenarioResult:
    ...
    def distributions(self):
        return TemporalDistribution.objects.filter(
            id__in=[
                ad["distribution"]
                for ad in LayerAggregatedDistribution.objects.filter(
                    layer__in=self.layers
                )
                .values("distribution")
                .distinct()
            ]
    ...
    def total_production(self):
        ...
        total_production = DataSet(
            label="Total production", data={"Total": production_value}, unit=unit
        )
    ...
    def total_production_per_feedstock(self):
        ...
        production = DataSet(
            label="Total production per feedstock", data=data, unit=unit
        )
        return production
    ...
    def get_charts(self):
        ...
        charts.update({"productionPerFeedstockBarChart": chart.as_old_dict()})
                ...
                charts.update(
                    {
                        chart_id: BarChart(
                            id=chart_id,
                            title="Production per component: "
                            + group_setting.group.name,
                            data=data,
                        ).as_old_dict()
                    }
                )
            ...
            "timesteps": [
                {
                    ...
                    "composition": {
                        "materials": [
                            {"name": feedstock, "amount": amount, "unit": "Mg/a"}
