...
from .views import ApproveItemView, RejectItemView, SubmitForReviewView, WithdrawFromReviewView
...
class ReviewWorkflowModelTests(TestCase):
    ...
    def test_submit_for_review(self):
        ...
        # Initial state
        self.assertEqual(self.collection.publication_status, UserCreatedObject.STATUS_PRIVATE)
        ...
        self.assertEqual(self.collection.publication_status, UserCreatedObject.STATUS_REVIEW)
    ...
    def test_approve(self):
        ...
        self.assertEqual(self.collection.publication_status, UserCreatedObject.STATUS_PUBLISHED)
    ...
    def test_reject(self):
        ...
        # Set up initial state
        self.collection.publication_status = UserCreatedObject.STATUS_REVIEW
        self.collection.submitted_at = '2025-01-01'
        self.collection.save()
...
class ReviewWorkflowPermissionTests(TestCase):
    ...
    def test_submit_permission(self):
        ...
        self.assertTrue(self.permission_checker.has_submit_permission(request, self.private_collection))
        
        # Regular user should not be able to submit someone else's private object
        request.user = self.regular_user
        self.assertFalse(self.permission_checker.has_submit_permission(request, self.private_collection))
        
        # No one should be able to submit an object already in review
        request.user = self.owner
        self.assertFalse(self.permission_checker.has_submit_permission(request, self.review_collection))
    ...
    def test_withdraw_permission(self):
        ...
        request = self.factory.post('/')
        request.user = self.owner
        self.assertTrue(self.permission_checker.has_withdraw_permission(request, self.review_collection))
        
        # Regular user should not be able to withdraw someone else's object
        request.user = self.regular_user
        self.assertFalse(self.permission_checker.has_withdraw_permission(request, self.review_collection))
        
        # No one should be able to withdraw a private object
        request.user = self.owner
        self.assertFalse(self.permission_checker.has_withdraw_permission(request, self.private_collection))
    ...
    def test_approve_permission(self):
        ...
        self.assertTrue(self.permission_checker.has_approve_permission(request, self.review_collection))
        
        # Owner should not be able to approve their own object
        request.user = self.owner
        self.assertFalse(self.permission_checker.has_approve_permission(request, self.review_collection))
        ...
        request.user = self.moderator
        self.assertFalse(self.permission_checker.has_approve_permission(request, self.private_collection))
    ...
    def test_reject_permission(self):
        ...
        request = self.factory.post('/')
        request.user = self.moderator
        self.assertTrue(self.permission_checker.has_reject_permission(request, self.review_collection))
        
        # Owner should not be able to reject their own object
        request.user = self.owner
        self.assertFalse(self.permission_checker.has_reject_permission(request, self.review_collection))
        ...
        request.user = self.moderator
        self.assertFalse(self.permission_checker.has_reject_permission(request, self.private_collection))
...
class ReviewWorkflowViewTests(TestCase):
    ...
    def test_submit_for_review_view(self):
        ...
        url = reverse('moderation:submit_for_review', kwargs={
        ...
        self.assertEqual(self.private_collection.publication_status, UserCreatedObject.STATUS_REVIEW)
        ...
        # Refresh from database
        self.private_collection.refresh_from_db()
        self.assertEqual(self.private_collection.publication_status, UserCreatedObject.STATUS_PRIVATE)
    
    def test_withdraw_from_review_view(self):
        """Test the withdraw from review view."""
        url = reverse('moderation:withdraw_from_review', kwargs={
            'content_type_id': self.content_type_id,
        ...
        self.assertEqual(self.review_collection.publication_status, UserCreatedObject.STATUS_PRIVATE)
        ...
        # Refresh from database
        self.review_collection.refresh_from_db()
        self.assertEqual(self.review_collection.publication_status, UserCreatedObject.STATUS_REVIEW)
    
    def test_approve_view(self):
        """Test the approve view."""
        url = reverse('moderation:approve_item', kwargs={
            'content_type_id': self.content_type_id,
            'object_id': self.review_collection.id
        })
        ...
        # Refresh from database
        self.review_collection.refresh_from_db()
        self.assertEqual(self.review_collection.publication_status, UserCreatedObject.STATUS_PUBLISHED)
    ...
    def test_reject_view(self):
        ...
        url = reverse('moderation:reject_item', kwargs={
            'content_type_id': self.content_type_id,
            'object_id': self.review_collection.id
        })
