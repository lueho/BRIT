...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not user_created_models:
            self.stdout.write(self.style.WARNING('No models inheriting from UserCreatedObject found.'))
            return
        ...
        self.stdout.write(self.style.SUCCESS('Publication inconsistency check complete.'))
    ...
    def process_model(self, model_class, dry_run):
        ...
        if published_missing_approval.exists():
            ...
            self.stdout.write(
                self.style.WARNING(f"Found {count} published {model_name} with missing approval metadata")
            )
            ...
            self.stdout.write(self.style.SUCCESS(f"No published {model_name} with missing approval metadata"))
        ...
        if review_missing_submission.exists():
            ...
            self.stdout.write(
                self.style.WARNING(f"Found {count} {model_name} in review with missing submission metadata")
            )
            ...
            self.stdout.write(self.style.SUCCESS(f"No {model_name} in review with missing submission metadata"))
        ...
        if invalid_submission.exists():
            ...
            self.stdout.write(self.style.SUCCESS(f"No {model_name} with submission metadata but not in review"))
            
        # Final check for any objects with invalid approval metadata
        if not invalid_approval.exists():
            self.stdout.write(self.style.SUCCESS(f"No {model_name} with invalid approval metadata"))
