...
class HamburgRoadsideTreeCatchmentAutocompleteViewTests(ViewWithPermissionsTestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.hamburg_catchment = Catchment.objects.create(name='Hamburg', region=region, publication_status='published')
        cls.dataset = GeoDataset.objects.create(
            name='Hamburg Roadside Trees',
            description='Roadside trees in Hamburg',
            model_name='HamburgRoadsideTrees',
        ...
        inside_3 = GeoPolygon.objects.create(geom='MULTIPOLYGON(((30 30, 30 70, 70 70, 70 30, 30 30)))')
        inside_region_3 = Region.objects.create(name='Inside 3', country='DE', borders=inside_3)
        cls.inside_outsider_catchment = Catchment.objects.create(name='Inside 3', region=inside_region_3,
                                                                 owner=cls.outsider)

        # Create a published region within dataset borders owned by outsider user
        inside_4 = GeoPolygon.objects.create(geom='MULTIPOLYGON(((40 40, 40 60, 60 60, 60 40, 40 40)))')
        inside_region_4 = Region.objects.create(name='Inside 4', country='DE', borders=inside_4)
        cls.inside_outsider_catchment_published = Catchment.objects.create(name='Inside 4', region=inside_region_4,
                                                                           owner=cls.outsider,
                                                                           publication_status='published')
        ...
        outside_2 = GeoPolygon.objects.create(geom='MULTIPOLYGON(((400 400, 400 500, 500 500, 500 400, 400 400)))')
        outside_region_2 = Region.objects.create(name='Outside 2', country='DE', borders=outside_2)
        cls.outside_outsider_catchment = Catchment.objects.create(name='Outside 2', region=outside_region_2,
                                                                  owner=cls.outsider)
    ...
    def test_only_inside_and_published_catchments_in_initial_queryset(self):
        ...
        self.assertEqual(
            [
                ...
                {'id': f'{self.inside_outsider_catchment_published.id}', 'selected_text': 'Inside 4',
                 'text': 'Inside 4'}
            ],
    ...
    def test_all_owned_and_published_catchments_visible_to_outsider(self):
        ...
        self.assertEqual(
            [
                ...
                {'id': f'{self.inside_catchment_1.id}', 'selected_text': 'Inside 1', 'text': 'Inside 1'},
                {'id': f'{self.inside_catchment_2.id}', 'selected_text': 'Inside 2', 'text': 'Inside 2'},
                {'id': f'{self.inside_outsider_catchment.id}', 'selected_text': 'Inside 3', 'text': 'Inside 3'},
                {'id': f'{self.inside_outsider_catchment_published.id}', 'selected_text': 'Inside 4',
                 'text': 'Inside 4'}
            ],
            list(response.json()['results'])
