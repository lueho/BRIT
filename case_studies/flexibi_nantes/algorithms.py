...
class InventoryAlgorithms(InventoryAlgorithmsBase):
    ...
    def nantes_greenhouse_production(cls, **kwargs):
        ...
        for greenhouse_type in greenhouse_types:
            clipped_filtered = clipped_filtered.union(
                NantesGreenhouses.objects.filter(**greenhouse_type.filter_kwargs)
            )
        ...
        result["aggregated_values"].append(
            {"name": "Total growth area", "value": total_surface, "unit": "ha"}
        )

        result["aggregated_values"].append(
            {"name": "Total production", "value": total_production, "unit": "Mg/a"}
        )
