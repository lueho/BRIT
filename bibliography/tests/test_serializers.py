...
class SourceSerializerTest(TestCase):
    def setUp(self):
        self.licence_data = {
            "name": "Test Licence",
            "reference_url": "http://example.com/licence",
            "description": "A test licence",
        }
        self.author_data = [
            {"first_names": "John", "last_names": "Doe"},
        ...
        with mute_signals(post_save):
            source = Source.objects.create(
                ...
                **{
                    ...
                    if key != "authors" and key != "licence"
                },
            )
...
class AuthorModelSerializerTestCase(TestCase):
    ...
    def test_field_content(self):
        ...
        self.assertEqual(
            data["preferred_citation"], self.author_attributes["preferred_citation"]
        )

    def test_bibtex_name_field(self):
        expected_bibtex_name = "Kennedy, J. F., Jr."
        self.assertEqual(self.serializer.data["bibtex_name"], expected_bibtex_name)

    def test_abbreviated_full_name_field(self):
        expected_abbreviated_name = "Kennedy, J. F., Jr."
        self.assertEqual(
            self.serializer.data["abbreviated_full_name"], expected_abbreviated_name
        )
...
class HyperlinkedLicenceSerializerTestCase(TestCase):
    ...
    def setUpTestData(cls):
        cls.licence = Licence.objects.create(
            name="Test Licence", reference_url="https://www.licence.com"
        )

    def test_data_rep(self):
        request = RequestFactory().get(
            reverse("licence-detail", kwargs={"pk": self.licence.pk})
        )
        serializer = HyperlinkedLicenceSerializer(
            self.licence, context={"request": request}
        )
        expected = {"name": self.licence.name, "url": request.build_absolute_uri()}
...
class HyperlinkedAuthorSerializerTestCase(TestCase):
    ...
    def test_data_rep(self):
        ...
        serializer = HyperlinkedAuthorSerializer(
            self.author, context={"request": request}
        )
        expected = {
...
class HyperlinkedSourceSerializerTestCase(TestCase):
    ...
    def test_data_rep(self):
        ...
        serializer = HyperlinkedSourceSerializer(
            self.source, context={"request": request}
        )
        ...
        expected = {
            ...
            "licence": OrderedDict(
                [
                    ...
                    (
                        ...
                        factory.get(
                            reverse(
                                "licence-detail", kwargs={"pk": self.source.licence.pk}
                            )
                        ).build_absolute_uri(),
                    ),
                ]
            ...
            "doi": OrderedDict(
                [
                    ("name", self.source.doi),
                    ("url", f"https://doi.org/{self.source.doi}"),
                ]
            ),
