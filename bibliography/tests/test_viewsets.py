...
class AuthorViewSetPermissionTestCase(ViewSetWithPermissionsTestCase):
    ...
    def test_update_401_unauthorized_for_anonymous(self):
        response = self.client.put(reverse('api-author-detail', args=[self.author.id]), self.author_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_delete_401_unauthorized_for_anonymous(self):
        response = self.client.delete(reverse('api-author-detail', args=[self.author.id]))
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
