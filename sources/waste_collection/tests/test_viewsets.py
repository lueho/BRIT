...
class CollectionSystemChangeViewSetTests(APITestCase):
    """Tests for collection-system-change atlas endpoint."""

    endpoint = "/waste_collection/api/waste-atlas/collection-system-change/"

    @classmethod
    def setUpTestData(cls):
        cls.region_de = Region.objects.create(name="Change Region DE", country="DE")
        cls.d2d = CollectionSystem.objects.create(name="Door to door")
        cls.bring_point = CollectionSystem.objects.create(name="Bring point")
        cls.no_collection = CollectionSystem.objects.create(
            name="No separate collection"
        )
        cls.bio = WasteCategory.objects.create(name="Biowaste")
        cls.food = WasteCategory.objects.create(name="Food waste")

        cls.catchment_changed = CollectionCatchment.objects.create(
            name="Changed Catchment", region=cls.region_de
        )
        cls.catchment_new = CollectionCatchment.objects.create(
            name="New Catchment", region=cls.region_de
        )
        cls.catchment_removed = CollectionCatchment.objects.create(
            name="Removed Catchment", region=cls.region_de
        )
        cls.catchment_no_change = CollectionCatchment.objects.create(
            name="No Change Catchment", region=cls.region_de
        )
        cls.catchment_other_country = CollectionCatchment.objects.create(
            name="Other Country Catchment",
            region=Region.objects.create(name="SE", country="SE"),
        )

        # 2023 baseline
        Collection.objects.create(
            name="Changed 2023",
            catchment=cls.catchment_changed,
            waste_category=cls.bio,
            collection_system=cls.d2d,
            valid_from=date(2023, 1, 1),
        )
        Collection.objects.create(
            name="Removed 2023",
            catchment=cls.catchment_removed,
            waste_category=cls.bio,
            collection_system=cls.d2d,
            valid_from=date(2023, 1, 1),
        )
        Collection.objects.create(
            name="No Change 2023",
            catchment=cls.catchment_no_change,
            waste_category=cls.bio,
            collection_system=cls.d2d,
            valid_from=date(2023, 1, 1),
        )
        Collection.objects.create(
            name="Other Country 2023",
            catchment=cls.catchment_other_country,
            waste_category=cls.bio,
            collection_system=cls.d2d,
            valid_from=date(2023, 1, 1),
    ...
    def test_returns_all_change_types(self):
        response = self.client.get(
            self.endpoint, {"country": "DE", "from_year": 2023, "to_year": 2024}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        by_catchment = {row["catchment_id"]: row for row in response.data}
        self.assertEqual(
            by_catchment[self.catchment_changed.id]["change_type"], "changed"
        )
        self.assertEqual(
            by_catchment[self.catchment_changed.id]["from_system"], "Door to door"
        )
        self.assertEqual(
            by_catchment[self.catchment_changed.id]["to_system"], "Bring point"
        )
        self.assertEqual(by_catchment[self.catchment_new.id]["change_type"], "new")
        self.assertEqual(
            by_catchment[self.catchment_removed.id]["change_type"], "removed"
        )
        self.assertEqual(
            by_catchment[self.catchment_no_change.id]["change_type"], "no_change"
        )

    def test_omits_catchments_with_no_data_in_either_year(self):
        response = self.client.get(
            self.endpoint, {"country": "DE", "from_year": 2023, "to_year": 2024}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        catchment_ids = {row["catchment_id"] for row in response.data}
        self.assertNotIn(self.catchment_other_country.id, catchment_ids)

    def test_falls_back_to_year_param_for_to_year(self):
        response = self.client.get(
            self.endpoint, {"country": "DE", "from_year": 2023, "year": 2024}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        by_catchment = {row["catchment_id"]: row for row in response.data}
        self.assertEqual(
    ...
    def test_country_filter_excludes_other_countries(self):
        response = self.client.get(
            self.endpoint, {"country": "SE", "from_year": 2023, "to_year": 2024}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        catchment_ids = {row["catchment_id"] for row in response.data}
        self.assertIn(self.catchment_other_country.id, catchment_ids)
        self.assertNotIn(self.catchment_changed.id, catchment_ids)

    def test_invalid_year_defaults(self):
        response = self.client.get(
            self.endpoint, {"country": "DE", "from_year": "bad", "to_year": "data"}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        # With default years 2023/2024, only no-change and other-country catchments have data
        by_catchment = {row["catchment_id"]: row for row in response.data}
        self.assertEqual(
            by_catchment[self.catchment_no_change.id]["change_type"], "no_change"
        )
