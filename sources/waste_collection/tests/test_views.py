...
class WasteFlyerListCheckUrlsViewTestCase(ViewWithPermissionsTestCase):
    ...
    @patch("sources.waste_collection.views.check_wasteflyer_urls.delay")
...
class WasteFlyerListCheckUrlsProgressViewTestCase(ViewWithPermissionsTestCase):
    ...
    @patch("sources.waste_collection.views.AsyncResult")
...
class WasteAtlasMapViewsTestCase(TestCase):
    ...
    def test_overview_directory_context_is_registry_driven(self):
        ...
        # Every group has a stable id and at least one region or europe map.
        for group in groups:
            with self.subTest(group=group["id"]):
                self.assertIn("id", group)
                self.assertIn("label", group)
                self.assertTrue(
                    group.get("regions") or group.get("europe_maps"),
                    f"group {group['id']} has no entries",
                )
        # At least one dedicated map-set region (e.g. DE) lists its own route.
...
@patch("sources.waste_collection.tests.test_views.check_wasteflyer_urls.apply")
@patch("sources.waste_collection.tests.test_views.chord")
...
@patch("sources.waste_collection.tasks.chord")
...
@patch("sources.waste_collection.tasks.find_wayback_snapshot_for_year")
@patch("sources.waste_collection.tasks.check_url")
