...
class PlantationYearFilter(NullableRangeFilter):
    def set_min_max(self):
        self.range_min = HamburgRoadsideTrees.objects.all().aggregate(
            min_year=Min("pflanzjahr")
        )["min_year"]
        self.range_max = HamburgRoadsideTrees.objects.all().aggregate(
            max_year=Max("pflanzjahr")
        )["max_year"]
...
class StemCircumferenceFilter(NullableRangeFilter):
    def set_min_max(self):
        self.range_min = HamburgRoadsideTrees.objects.all().aggregate(
            min_circ=Min("stammumfang")
        )["min_circ"]
        self.range_max = HamburgRoadsideTrees.objects.all().aggregate(
            max_circ=Max("stammumfang")
        )["max_circ"]
...
class HamburgRoadsideTreesFilterSet(BaseCrispyFilterSet):
    ...
    gattung_deutsch = MultipleChoiceFilter(
        widget=CheckboxSelectMultiple,
        choices=GATTUNG_CHOICES,
        label="Tree genus",
        method="filter_genus",
    )
    plantation_year = PlantationYearFilter(
        field_name="pflanzjahr", label="Plantation year"
    )
    stem_circumference = StemCircumferenceFilter(
        field_name="stammumfang", label="Stem circumference [cm]"
    )
    ...
    def filter_genus(qs, _, value):
        if "Other" in value:
            qs = qs.exclude(
                gattung_deutsch__in=[
                    choice[0] for choice in GATTUNG_CHOICES if choice[0] not in value
                ]
            )
        else:
...
__all__ = [
    "GATTUNG_CHOICES",
    "BEZIRK_CHOICES",
    "HamburgRoadsideTreeFilterFormHelper",
    "PlantationYearFilter",
    "StemCircumferenceFilter",
