...
class GlobalObjectViewSetTestCase(TestCase):
    def setUp(self):
        ...
        self.example_object = TestGlobalObject.objects.create(
            name="Test Object", description="A test global object."
        )
    ...
    def get_mock_viewset(self, method, user=None, data=None, pk=None):
        ...
        with patch.object(
            ...
            with patch.object(
                GlobalObjectViewSet, "get_serializer_class", return_value=MockSerializer
            ):
    ...
    def test_write_access_unauthenticated_user(self):
        ...
        response = self.get_mock_viewset(
            "post", data={"name": "New Object", "description": "New description."}
        )
        ...
        response = self.get_mock_viewset(
            "delete", user=AnonymousUser(), pk=self.example_object.id
        )
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_write_access_authenticated_regular_user(self):
        ...
        response = self.get_mock_viewset(
            "delete", user=self.regular_user, pk=self.example_object.id
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_write_access_authenticated_staff_user(self):
        ...
        self.assertEqual(
            TestGlobalObject.objects.get(id=response.data["id"]).name,
            "Staff Created Object",
        )
        ...
        response = self.get_mock_viewset(
            "delete", user=self.staff_user, pk=self.example_object.id
        )
    ...
    def test_create_object_with_duplicate_name_staff_user(self):
        ...
        self.assertIn(
            "name", response.data, "Response should contain 'name' field error."
        )
    ...
    def test_delete_nonexistent_object_staff_user(self):
        ...
        nonexistent_pk = 9999
        response = self.get_mock_viewset(
            "delete", user=self.staff_user, pk=nonexistent_pk
        )
        self.assertEqual(
            response.status_code,
            status.HTTP_404_NOT_FOUND,
            "Nonexistent object should return 404.",
        )
...
class AutoPermModelViewSetTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.viewset.get_queryset = MagicMock(
            return_value=MagicMock(model=self.model_cls)
        )
