...
from ..widgets import RangeSliderWidget, NullableRangeSliderWidget
...
class RangeSliderWidgetTestCase(TestCase):
    ...
    def test_widget_get_context_with_subwidgets_ids(self):
        ...
        for swx, subwidget in enumerate(context['widget']['subwidgets']):
            self.assertEqual(subwidget['attrs']['id'], base_id + "_" + widget.suffixes[swx])

    def test_widget_get_context_with_unit_in_value_text(self):
        widget = RangeSliderWidget(attrs={'data-range_min': 0, 'data-range_max': 100, 'data-unit': 'm'})
        context = widget.get_context('test_widget', [50, 100], None)
...
class NullableRangeSliderWidgetTestCase(TestCase):
    def test_init(self):
        widget = NullableRangeSliderWidget(attrs={'data-range_min': 0, 'data-range_max': 100, 'data-is_null': 'true'})
