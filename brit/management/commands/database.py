...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "action", type=str, help="Defines the action to be taken on the database"
        )
    ...
    def capture():
        ...
        """
        os.system(
            f"heroku pg:backups:capture {os.environ.get('HEROKU_POSTGRESQL_COLOR_URL')}"
        )
    ...
    def download(file_name):
        ...
        """
        os.system(
            f"heroku pg:backups:download -a {os.environ.get('HEROKU_APP_NAME')} -o {file_name}"
        )
    ...
    def dump_local_db(file_name):
        ...
        os.system(
            f"pg_dump -Fc --no-acl --no-owner -h db -p 5432 -U {user} > {file_name}"
        )
    ...
    def restore_local_db(file_name):
        ...
        os.environ["PGPASSWORD"] = os.environ.get("POSTGRES_PASSWORD")
        os.system(
            f"pg_restore --clean --no-acl --no-owner -h {host} -p {port} -U {user} -d {database} {file_name}"
        )
    ...
    def restore_production_db(self, file_name):
        ...
        os.system(
            f"heroku pg:backups:restore -a {app_name} --confirm {app_name} '{url}' {db_url}"
        )
