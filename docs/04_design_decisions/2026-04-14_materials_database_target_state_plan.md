...
- Phase 4b - add a mismatch report or management command that compares raw-derived output with persisted fallback rows before destructive cleanup is planned
  - completed by `report_composition_normalization_mismatches`
  - the command can be run for all samples or selected `--sample-id` values, can emit summary-only output, and can fail when mismatches exist
...
- Phase 4b has started with mismatch reporting; the next work should constrain remaining write surfaces and then continue the usage inventory because direct `Composition` and `WeightShare` references still include legitimate settings behavior, legacy normalized-value behavior, and compatibility API behavior.
...
1. make the remaining `WeightShare` editing surfaces explicitly present themselves as saved normalized-composition compatibility workflows, not as the preferred path for new component observations
2. continue the Phase 4b inventory of remaining `Composition` and `WeightShare` consumers, classified by settings, read, write, import/export, and compatibility API behavior
3. migrate the next primary read surfaces to `get_sample_normalized_compositions()` where they still use direct persisted normalized values
4. define and enforce the write-side boundary so new component observations use `ComponentMeasurement` while `WeightShare` writes are limited to explicit legacy compatibility workflows

That slice is now the smallest high-value step because mismatch reporting exists and can support later cleanup decisions. The largest remaining architectural mismatch with the report is the write-side and compatibility-surface dependence on persisted normalized `WeightShare` rows.
